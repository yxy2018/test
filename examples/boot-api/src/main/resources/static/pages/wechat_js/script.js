...
function jcalendar_week(options){
	...
	var setdaydata = function(year,weeknum){
		...
		calendarid.find(".calendar_info").html(trueweeknum[0]+'年第'+trueweeknum[1]+'周');
		...
		for(var i=0;i<7;i++){
			...
			week_day+='<li class="calendar_day'+dayaddclass+'" '
                +'ontouchend="gtouchend(\''+newdate.getFullYear()+'-'+(newdate.getMonth()+1)+'-'+newdate.getDate()+'\',this)" '
				+'ontouchstart="gtouchstart(\''+newdate.getFullYear()+'-'+(newdate.getMonth()+1)+'-'+newdate.getDate()+'\',this)" '+
